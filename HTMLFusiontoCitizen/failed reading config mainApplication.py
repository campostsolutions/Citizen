...
class selectCitizenPostCommandCreatedHandler(adsk.core.CommandCreatedEventHandler):
    ...
    def notify(self, args):
        try:
            ...
            load_from_file_button = groupChildInputs1.addButtonDefinition('load_posts_from_file', 'Reload Posts from File')
...
# Toggle visibility based on post type
            ...
            elif changedInput.id == 'load_posts_from_file':
                try:
                    file_path = os.path.join(os.path.dirname(__file__), 'machines.txt')
                    if not os.path.exists(file_path):
                        ui.messageBox(f"'machines.txt' not found at: {file_path}")
                        return

                    with open(file_path, 'r') as f:
                        valid_posts = [line.strip().lower() for line in f if line.strip()]

                    def filter_list(post_list):
                        return [p for p in post_list if p.lower().replace('.cps', '') in valid_posts]

                    L1_posts = filter_list(L1_posts)
                    L2_posts = filter_list(L2_posts)
                    L3_posts = filter_list(L3_posts)
                    L320_posts = filter_list(L320_posts)
                    Miyano_posts = filter_list(Miyano_posts)
                    other_posts = filter_list(other_posts)

                    populate_posts_dropdown(posts_fldr, select_post_dropdown.listItems, select_post_dropdown)

                    ui.messageBox(f"Post list updated from machines.txt ({len(valid_posts)} items read).")
                except Exception as err:
                    ui.messageBox(f"Failed to load posts from file:\n{err}")    
        except Exception as err:
            if ui:
                ui.messageBox(err.args[0])
