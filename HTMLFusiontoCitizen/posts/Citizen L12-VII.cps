...
description = "Alkart Wizard L12-VII";
vendor = "Citizen";
...
setMachineStyle = function (tool) {
    ...
    if (tool.number >= 23 && tool.number <= 30 || tool.number >= 15 && tool.number <= 20 || tool.number >= 35) {
    ...
    if (currentSection.spindle == SPINDLE_PRIMARY) {
        ...
        } else if (tool.number >= 7 && tool.number <= 9) {
            if ((currentSection.getType() == TYPE_MILLING)) {
