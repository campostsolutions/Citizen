...
function setScaling(toolNumber) {
  ...
  const offset180 = (toolNumber >= 21 && toolNumber <= 23) && crossWorking;
  const offset90 = ((toolNumber >= 11 && toolNumber <= 14) || (toolNumber >= 21 && toolNumber <= 23) && !offset180);
...
function setWorkPlane(abc) {
  ...
  if (gotBAxis) {
    ...
    if (generalSettings.newGen && hasParameter("operation:isMultiAxisStrategy") && getParameter("operation:isMultiAxisStrategy") != 0){
      ...
      yOutput = createVariable({ prefix: "Y" }, yFormat);
		  ...
		  zOutput.reset();
		  writeBlock(gFormat.format(920), generalSettings.safetyVal + "+" + addTo814, yOutput.format(0), zOutput.format((-3.0 - tool.diameter / 2)*-1), bOutput.format(abc.y), cOutput.format(0));
