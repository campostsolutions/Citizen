...
longDescription = "Interface L32-XII with Akart CNC Wizard";
...
setMachineStyle = function (tool) {
  if (debugMode) {
    ...
    if (tool.number >= 24 && tool.number <= 30 || tool.number >= 14  && tool.number <= 20 ){
  ...
  if (currentSection.spindle == SPINDLE_PRIMARY) {
    if (tool.number >= 1 && tool.number <= 6) {
      ...
      if (getMachiningDirection(currentSection) == MACHINING_DIRECTION_RADIAL) {
        error(localize("Invalid orientation for " + generalSettings.toolpost + " toolpost"))
      }
    ...
    else if (tool.number >= 31 && tool.number <= 39) {
    ...
    if (tool.number >= 31 && tool.number <= 39) {
      if (currentSection.getType() == TYPE_TURNING ||(tool.liveTool == false)) {
        ...
        if ((currentSection.getType() == TYPE_MILLING)) {
          if (getMachiningDirection(currentSection) == MACHINING_DIRECTION_RADIAL && (tool.number <= 34 && tool.number >= 31)) {
