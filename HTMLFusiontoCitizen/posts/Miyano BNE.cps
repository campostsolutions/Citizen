...
properties = {
  ...
  miyano_maximumSpindleSpeed    : 3000, // specifies the maximum spindle speed
...
function defineMachine() {
  gotSecondarySpindle = properties.gotSecondarySpindle;
...
function onSection() {
  ...
  if (operationSupportsTCP) {
    ...
    if (machineState.usePolarCoordinates) {
      ...
      writeBlock(gMotionModal.format(0), zOutput.format(initialPosition.z));
      writeBlock(gMotionModal.format(0), xOutput.format(initialPosition.x), machineState.isTurningOperation ? "" : yOutput.format(initialPosition.y));
...
function goHome() {
  ...
  writeBlock(gFormat.format(0), "U" + xFormat.format(0), "V" + xFormat.format(0), "W" + xFormat.format(0), "T00")
