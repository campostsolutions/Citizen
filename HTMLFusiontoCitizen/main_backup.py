    ...
    if cam:
        ...
        try:
            ...
            if os.path.exists(logPath):
                os.remove(logPath)
            # Check units from form data or default to mm
            units = adsk.cam.PostOutputUnitOptions.MillimetersOutput
            # Note: Units are now handled via HTML form data, defaulting to mm
...
class HTMLPaletteIncomingHandler(adsk.core.HTMLEventHandler):
    ...
    def notify(self, args):
        try:
            ...
            if data_source.startswith('TRIGGER_PYTHON_PROCESSING:'):
                ...
                try:
                    ...
                    if "selectedImage" in data:
                        ...
                        print(f"Processing form data with selectedImage: {selected_machine}")
                    else:
                        # Old format with nested structure
            selected_machine = data.get("selectedImage")
            form = data.get("formValues", {})
                        print(f"Processing old format with selectedImage: {selected_machine}")
            ...
            if not any([L1_posts, L2_posts, L3_posts, L320_posts, Miyano_posts, other_posts]):
                # Populate post lists if they're empty
                posts_fldr = os.path.join(os.path.dirname(__file__), 'posts')
