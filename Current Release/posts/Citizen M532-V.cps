...
description = "Alkart Wizard M532-V";
vendor = "Citizen";
...
longDescription = "Interface M532-V with Akart CNC Wizard";
...
setMachineStyle = function (tool) {
  ...
  } else if (currentSection.spindle != SPINDLE_PRIMARY && tool.number <= 20) {
    error(localize("Tool " + tool.number + " is not available on the Sub Spindle!"));
  }

  if (tool.number == 30 || tool.number >= 15 && tool.number <= 19 || tool.number > 35 || tool.number == 10) {
  ...
  if (currentSection.spindle == SPINDLE_PRIMARY) {
    if (tool.number >= 1 && tool.number <= 5) {
      if (currentSection.getType() == TYPE_MILLING) {
        errormessage();
      } else {
    ...
    } else if (tool.number >= 6 && tool.number <= 9) {
      if ((currentSection.getType() == TYPE_MILLING)) {
    ...
    } else if (tool.number >= 20 && tool.number <= 29) {
      //generalSettings = oppositeEndWorking2;
      if ((currentSection.getType() == TYPE_MILLING) && (tool.liveTool == true)) {
        if (getMachiningDirection(currentSection) == MACHINING_DIRECTION_RADIAL) {
          generalSettings = turretCrossWorking;
        } else {
          generalSettings = turretEndWorking1;
          if (debugMode) {
            writeBlock("Main Spindle Turret End Working - MILLING");
            writeBlock("Tool is :- " + tool.number);
          }
        }
      } else {
        generalSettings = turretEndWorking2;
        if (debugMode) {
          writeBlock("Main Spindle Turret End Working - TURNING");
          writeBlock("Tool is :- " + tool.number);
        }
      }
    }
    ...
    if (tool.number >= 31 && tool.number <= 35) {
      if (currentSection.getType() == TYPE_TURNING || currentSection.getParameter("operation-strategy") == "drill") {
        ...
        if (debugMode) {
          writeBlock("Sub Spindle Back Tool Post Working - TURNING");
          writeBlock("Tool is :- " + tool.number);
        }
      } else {
        error(localize("Milling Operations are not supported with this tool number"));
      }
      ...
      if (currentSection.getType() == TYPE_TURNING || currentSection.getParameter("operation-strategy") == "drill") {
        generalSettings = oppositeEndWorking2;
        if (debugMode) {
          writeBlock("Sub Spindle Turret Working - TURNING");
          writeBlock("Tool is :- " + tool.number);
        }
      } else {
        if (getMachiningDirection(currentSection) == MACHINING_DIRECTION_RADIAL) {
...
setupMachine = function () {
  ...
  gotYAxis = (tool.number >= 20 && tool.number <= 29 || tool.number >= 6 && tool.number <= 14 && !currentSection.getType() == TYPE_TURNING);
