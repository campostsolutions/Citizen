...
description = "Alkart Wizard L32-VIII";
vendor = "Citizen";
...
setMachineStyle = function (tool) {
    ...
    if (tool.number >= 27 && tool.number <= 30 || tool.number >= 14 && tool.number <= 20 || tool.number >= 36) {
    ...
    if (currentSection.spindle == SPINDLE_PRIMARY) {
        ...
        } else if (tool.number >= 21 && tool.number <= 24) {
    ...
    else if (tool.number >= 32 && tool.number <= 35) {
        if ((currentSection.getType() == TYPE_MILLING && tool.liveTool == true)) {
...
setupMachine = function () {
    ...
    gotYAxis = (tool.number >= 7 && tool.number <= 14 && !currentSection.getType() == TYPE_TURNING);
