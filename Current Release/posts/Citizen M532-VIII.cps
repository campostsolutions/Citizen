...
description = "Alkart Wizard M532-VIII";
vendor = "Citizen";
...
longDescription = "Interface M532-VIII with Akart CNC Wizard";
...
setMachineStyle = function (tool) {
  ...
  if (tool.number == 30 || tool.number >= 15 && tool.number <= 19 || tool.number > 39 || tool.number == 10) {
  ...
  if (currentSection.spindle == SPINDLE_PRIMARY) {
    ...
    if (tool.number >= 31 && tool.number <= 39) {
      if (currentSection.getType() == TYPE_TURNING || currentSection.getParameter("operation-strategy") == "drill") {
        ...
        if (tool.number <= 36 && tool.number >= 31) {
          if (getMachiningDirection(currentSection) == MACHINING_DIRECTION_RADIAL) {
    ...
    } else if (tool.number <= 33 && tool.number >= 31) {
...
setupMachine = function () {
  ...
  gotYAxis = (tool.number >= 20 && tool.number <= 29 || tool.number >= 6 && tool.number <= 14 || tool.number >= 31 && tool.number <= 39 && !currentSection.getType() == TYPE_TURNING);
