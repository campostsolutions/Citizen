...
description = "Alkart Wizard L20-VIII";
vendor = "Citizen";
...
longDescription = "Sample post to show how integration might work with the Citizen wizard";
...
setMachineStyle = function (tool) {
  ...
  // Alarm if range of tools aren't applicable for main or sub operations.
    ...
    if (tool.number >= 14 && tool.number <= 20 || tool.number >= 35) {
    ...
    if (currentSection.spindle == SPINDLE_PRIMARY) {
        ...
        } else if (tool.number >= 21 && tool.number <= 23)  {
        ...
        else if (tool.number >= 31 && tool.number <= 34) {
        ...
        if ((currentSection.getType() != TYPE_MILLING)) {
            if (getMachiningDirection(currentSection) == MACHINING_DIRECTION_RADIAL) {
                error(localize("Invalid orientation for " + generalSettings.toolpost + " toolpost - check" + getParameter("operation-comment")))
            }
