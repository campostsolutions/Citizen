description = "Alkart Wizard L32-X";
vendor = "Citizen";
...
longDescription = "Interface L212-X with Akart CNC Wizard";
...
setMachineStyle = function (tool) {
  ...
  if (tool.number >= 23 && tool.number <= 30 || tool.number >= 14 && tool.number <= 20 || tool.number >= 39) {
  ...
  if (currentSection.spindle == SPINDLE_PRIMARY) {
    if (tool.number >= 1 && tool.number <= 7) {
      if (currentSection.getType() == TYPE_MILLING) {
        errormessage();
      } else {
      ...
      if ((currentSection.getType() == TYPE_MILLING)) {
        ...
        //add g50 shift here
        generalSettings.g50Offset = 5;
      } else {
        errormessage();
      }
    } else if (tool.number >= 9 && tool.number <= 10) {
      if ((currentSection.getType() == TYPE_MILLING)) {
    ...
    } else if (tool.number >= 11 && tool.number <= 13) {
      if ((currentSection.getType() == TYPE_MILLING && (tool.liveTool == true))) {
        generalSettings = gangBaxisManual;
        if (debugMode) {
          writeBlock("Main Spindle Gang B Axis Manual");
          writeBlock("Tool is :- " + tool.number);
        }
      } else {
        error(localize("Static Operations are not supported with this tool number"));
      }
    } else if (tool.number >= 21 && tool.number <= 22) {
      generalSettings = oppositeEndWorking2;
      if ((currentSection.getType() == TYPE_MILLING && (tool.liveTool == true))) {
        error(localize("Driven Operations are not supported with this tool number"));
      } else {
        generalSettings = oppositeEndWorking2;
        if (debugMode) {
    ...
    if (tool.number >= 31 && tool.number <= 38) {
      if (currentSection.getType() == TYPE_TURNING || (tool.liveTool == false)) {
        ...
        if ((currentSection.getType() == TYPE_MILLING)) {
          if (tool.number <= 34 && tool.number >= 31) {
            ...
            error(localize("Invalid operation for T" + tool.number + " - check " + getParameter("operation-comment")));
          }
          if (debugMode) {
...
setupMachine = function () {
  ...
  gotYAxis = (tool.number >= 7 && tool.number <= 14 || tool.number >= 31 && tool.number <= 39 && !currentSection.getType() == TYPE_TURNING);
  ...
  var bAxisMain = createAxis({coordinate:1, table:false, axis:[0, -1, 0], range:[properties.increaseBAxisStroke ? 101.0 : 90.5, -45], preference:1});
  var cAxisMain = createAxis({coordinate:2, table:true, axis:[0, 0, -1], cyclic:true, range:[0, 99999], preference:0}); // C axis is modal between primary and secondary spindle
  var bAxisSub = createAxis({coordinate:1, table:false, axis:[0, -1, 0], range:[-45, properties.increaseBAxisStroke ? 101.0 : 90.5], preference:0});
