...
description = "Citizen Miyano BNE 51 MSY with Mitsubishi control";
...
longDescription = "Interface BNE 51 MSY with Akart CNC Wizard";
...
properties = {
  ...
  miyano_maximumSpindleSpeed    : 5000, // specifies the maximum spindle speed
...
function defineMachine() {
  gotSecondarySpindle = true;
  gotMultiTurret = properties.gotMultiTurret;
  ...
  xAxisMinimum = toPreciseUnit(-12.5, MM); // specifies the maximum range for the X-axis (RADIUS MODE VALUE)
...
function activateMachine(section) {
  ...
  if (section.getSpindle() == SPINDLE_PRIMARY) {
    ...
    cAxis = createAxis({coordinate:2, table:true, axis:[0, 0, 1], cyclic:true, reset:1, preference:0, tcp:true});
  } else {
...
function onSection() {
  ...
  if (tool.number >= 13 && tool.number <= 20 || tool.number > 32) {
    error(localize("Tool " + tool.number + " is not available on this Machine"));
  }
  ...
  //Apply 180 degree offset if lower turret
  if (tool.number >= 21) {
  ...
  if (operationSupportsTCP) {
    ...
    if (machineState.usePolarCoordinates) {
      ...
      writeBlock(gMotionModal.format(0), zOutput.format(initialPosition.z), machineState.isTurningOperation ? "" : yOutput.format(initialPosition.y));
      writeBlock(gMotionModal.format(0), xOutput.format(initialPosition.x));
  ...
  if ((maximumSpindleSpeed > 0) && (currentSection.getTool().getSpindleMode() == SPINDLE_CONSTANT_SURFACE_SPEED)) {
    //if (insertToolCall) {
    writeBlock(mFormat.format(getSection(0).spindle == SPINDLE_PRIMARY ? 497 : 495)); // turn fluctuation off for G96 interference
  ...
  if (!groupedOperations) {
    ...
    if (machineState.isTurningOperation) {
      ...
      if (tool.number <= 12) {
        ...
        if (getSpindle(PART) == SPINDLE_MAIN) {
          if (isPolarModeActive()) {
            writeBlock(mFormat.format(5), mFormat.format(246));
          } else {
            writeBlock(mFormat.format(246));
          }
        } else {
          if (isPolarModeActive()) {
            writeBlock(mFormat.format(105), mFormat.format(246));
          } else {
            writeBlock(mFormat.format(246));
          }
        }
    ...
    if ((tool.getSpindleMode() == SPINDLE_CONSTANT_SURFACE_SPEED)) {
      writeBlock(mFormat.format(getSection(0).spindle == SPINDLE_PRIMARY ? 496 : 494)); // turn fluctuation off for G96 interference
    }
    goHome();
    forcePolarCoordinates = false;
    forcePolarInterpolation = false;
    partCutoff = false;
    forceAny();
    //Flood Coolant upper turret - always turn off. operators responsibility to turn on.
    if (tool.number <= 12) {
      writeBlock(mFormat.format(29));
    } else if (tool.number >= 21 && tool.number <= 32) {
